...
def train(model, train_loader, val_loader, criterion, optimizer, device, epochs=20):
    ...
    for epoch in range(epochs):
        ...
        print("訓練階段:")
        train_pbar = tqdm(train_loader, desc=f"訓練 Epoch {epoch+1}/{epochs}")
        batch_times = []
        ...
        print("\n驗證階段:")
        val_pbar = tqdm(val_loader, desc=f"驗證 Epoch {epoch+1}/{epochs}")
        ...
        if val_accuracy > best_acc:
            ...
            checkpoint_path = os.path.join('checkpoints', 'wide_cnn_best.pth')
    ...
    print(f"最佳模型已保存至 'checkpoints/wide_cnn_best.pth'")
...
if __name__ == "__main__":
    ...
    # 初始化模型
    print("初始化WideCNN模型...")
    model = WideCNN(in_channels=3, num_classes=100).to(device)
    ...
    try:
        # 開始訓練
        best_acc = train(model, train_loader, val_loader, criterion, optimizer, device)
